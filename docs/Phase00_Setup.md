...
### 2-1. 설치된 도구 점검
초기 점검 당시에는 C++ 빌드 도구가 빠져 있었지만, 현재 PC에서는 필요한 도구가 설치되어 있고
`Debug|x64` 빌드까지 확인된 상태다.
...
| MSBuild | ✅ 설치됨 | 17.14 |
| Git | ✅ 설치됨 | 2.53 |
| **MSVC 컴파일러 (cl.exe)** | ✅ 설치됨 | 14.44.35207 |
| **Windows SDK** | ✅ 설치됨 | 10.0.26100.0, `d3d12.h` 확인 |

→ 다른 PC에서 이 프로젝트를 열었을 때 위 항목이 없다면 **"C++를 사용한 데스크톱 개발" 워크로드**를
설치해야 한다. 이 워크로드에 MSVC 컴파일러와 Windows SDK가 함께 포함된다.
...
- "MSVC가 없다" = `cl.exe`가 없다 = **C++ 코드를 .exe로 만들 방법이 아예 없다.**
  초기 Phase 0에서 빌드가 불가능했던 직접적인 이유다.
...
- [x] **"C++를 사용한 데스크톱 개발" 워크로드 설치 확인**
- [x] `Debug|x64` 빌드 성공 확인

다음: **Phase 1 실행 확인** → [Phase01_Win32Window.md](Phase01_Win32Window.md)
