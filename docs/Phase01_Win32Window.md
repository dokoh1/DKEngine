...
| `src/Core/Log.h` | `OutputDebugString` + Debug 콘솔 기반 로깅. |
| `src/Core/WindowsMinimal.h` | `Windows.h` 포함 정책(`WIN32_LEAN_AND_MEAN`, `NOMINMAX`). |

설계 원칙: **관심사 분리**. "창을 다루는 일"(Window)과 "게임을 굴리는 일"(Application)을 나눴다.
Phase 2에서 렌더러가 추가돼도 `Application`은 "창 + 렌더러"를 조율하고, D3D12 세부 객체는
`src/RHI/` 아래에 둔다.
...
- `Window`는 RAII 객체로 취급한다. 생성한 `HWND`와 등록한 윈도우 클래스는 `Window::Destroy()`와
  소멸자에서 정리해, 실패 경로와 정상 종료 경로가 같은 정리 코드를 타게 한다.
...
- `WM_NCDESTROY`에서 `HWND`에 저장한 `this` 포인터를 비워 둔다. 창이 사라진 뒤 잘못된 포인터로
  메시지를 처리하지 않게 하기 위한 정리 단계다.
...
`Application`은 렌더러를 호출만 하고, `src/RHI/`에 디바이스/스왑체인/커맨드 큐/펜스 래퍼가
추가된다. (로드맵의 "D3D12의 80%"라고 표시된 그 단계)
